...
export class ResultService {
  ...
  public validateShot(coordinate: Coordinate, board: Board): Result {
    ...
    if (!board.isCoordinateEmpty(board.Values[coordinate.rowIndex][coordinate.columnIndex])) {
      const shipName = board.Values[coordinate.rowIndex][coordinate.columnIndex];
  ...
  private checkIfShipSinkOrHit(shipName: string, board: Board): Result {
    ...
    if (this.hitShipsData.get(shipName) === board.Ships.find(s => s.name === shipName).size) {
